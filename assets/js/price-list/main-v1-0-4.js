import DynamicListings from "./dynamic-listings-v-1-0-1.js";
import StockStatus from "./stock-status-v-1-0-0.js";

jQuery(document).ready(function ($) {
  StockStatus.initialize();
  ...
  JetSmartFilters.events.subscribe(
    "fiters/remove",
    DynamicListings.addListingToResetQueue
  );

  JetSmartFilters.events.subscribe(
    "activeItems/change",
    DynamicListings.filtersApplyToListing
  );

  JetSmartFilters.events.subscribe(
    "ajaxFilters/start-loading",
    DynamicListings.hideListing
  );

  JetSmartFilters.events.subscribe(
    "ajaxFilters/end-loading",
    DynamicListings.showListing
  );
  ...
  setTimeout(() => {
    const counters = document.querySelectorAll(
      `[data-query-id="listing-grid-addons"] .jet-filter-row`
    );

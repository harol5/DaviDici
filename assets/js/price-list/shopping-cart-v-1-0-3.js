...
class ShoppingCartDOM {
  ...
  static updateProduct(productDomRef, total) {
    productDomRef.querySelector(
      ".total-container"
    ).innerText = `${USDollar.format(total)}`;
  }
  ...
  static updateShoppingCartContainer() {
    ...
    ShoppingCartDOM.grandTotalRef.innerText = `${USDollar.format(
    ...
    if (
      shoppingCart.getProducts().length > 0 &&
      ShoppingCartDOM.orderNowRef.classList.contains("disabled")
    ) {
      ShoppingCartDOM.orderNowRef.classList.remove("disabled");
    }

    if (shoppingCart.getProducts().length === 0)
      ShoppingCartDOM.orderNowRef.classList.add("disabled");
  }
    ...
    modal.innerHTML = `
        <div class="modal-content">
          ...
          <div class="modal-content-footer">
            <h3>Grand Total:</h3>
            <span id="grand-total">${USDollar.format(
              shoppingCart.getGrandTotal()
            )}</span>
  ...
  static initializeShoppingCart() {
    ...
    if (isShoppingCartSaved) {
      const { grandTotal, products, suggestedWashbasinSize, orderNowUrl } =
        JSON.parse(isShoppingCartSaved);
...
class ShoppingCart {
  ...
  updateUrl() {
    ...
    // this.#baseUrl = "https://seven.test/orders/create-so-num?SKU=";
    this.#baseUrl = "https://portal.davidici.com/orders/create-so-num?SKU=";
    let allSkuAndQty = "";
  ...
  updateProduct(index, quantity) {
    ...
    const isVanityOrSideUnit = product.categories.find(
      (cat) => cat === "vanities price list" || cat === "Side Units price list"
    );
  ...
  async addNewProduct(id, quantity) {
    ...
    await jQuery.ajax({
      ...
      success: function (product) {
        ...
        const isVanityOrSideUnit = product.categories.find(
          (cat) =>
            cat === "vanities price list" || cat === "Side Units price list"
        );
        if (isVanityOrSideUnit) {
          if (product.qty > 1) {
            for (let i = 0; i < product.qty; i++) {
              self.#suggestedWashbasinSize[isVanityOrSideUnit].push(
                product.size
              );
            }
          } else {
  ...
  removeProduct(index) {
    ...
    if (isVanityOrSideUnit) {
      for (let i = 0; i < product.qty; i++) {
        const index = this.#suggestedWashbasinSize[isVanityOrSideUnit].indexOf(
          product.size
        );
...
//this class manages input and buttons events and render conditions for qty input and add to cart button.
class ProductDOM {
  ...
  static initializeInputEventDelegation() {
    document.addEventListener("input", (e) => {
      ...
      if (input.classList.contains("qty")) {
        ...
        quantity > 0
          ? this.enableAddToCartButton(addToCartButton, quantity, qtyInputId)
          : this.disabledAddToCartButton(addToCartButton);
      }

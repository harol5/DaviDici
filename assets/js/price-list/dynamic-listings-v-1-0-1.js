export default class DynamicListings {
  ...
  static onFirstRender(event, xhr, settings) {
    ...
    if (isListing) {
      const listingSelector =
        xhr.responseJSON.data.indexer_data.provider.split("/")[1];
      ...
      const listingContainerRef = document.querySelector(
        `.${listingContainer}`
      );
    ...
    if (isWashbasin) DynamicListings.currentCollection.name = "washbasins";

    //applies size filter if page is washbasin and vanities and/or side units have being selected.
    // const isWashbasin = settings.url.includes("washbasins");
    const isFilterReady = settings.url.includes("wp-admin");
    if (
      isFilterReady &&
      DynamicListings.currentCollection.name === "washbasins" &&
      DynamicListings.currentCollection.isFirstRender
    ) {
      console.log("is washbasin consition true");
      ...
      if (vanitieSizes.length === 1 && sideUnitSizes.length === 0) {
        sizeFilter = document.querySelector(
          `.jet-checkboxes-list__row.jet-filter-row:not(.jet-filter-row-hide) .jet-checkboxes-list__input[data-label="${sizeSum}"]`
        );
      } else {
        sizeFilter = document.querySelector(
          `.jet-checkboxes-list__row.jet-filter-row:not(.jet-filter-row-hide) .jet-checkboxes-list__input[data-label="${sizeAsString.slice(
            0,
            -1
          )}(${sizeSum})"]`
        );
      ...
      if (!sizeFilter) {
        sizeFilter = document.querySelector(`[data-label="CUSTOM"]`);
      }
  ...
  static filtersApplyToListing(activedFilters, type, listingSelector) {
    ...
    DynamicListings.NumOfActiveListingFilters[`${item}-container`] =
      activedFilters.length;
  }

  static hideListing(type, listingSelector) {
    const { domRef, item, loadingSpinnerDomRef } =
      DynamicListings.cache[listingSelector];
    DynamicListings.addOverlayWithLoadingSpinnerToFilter(
      item,
      loadingSpinnerDomRef
    );

    //----adds class of filtered container to current filtered containers array.
    if (!DynamicListings.currentFilteredListings.includes(`${item}-container`))
      DynamicListings.currentFilteredListings.push(`${item}-container`);
  ...
  static showListing(type, listingSelector) {
    ...
    if (DynamicListings.resetedFiltersQueue.length !== 0) {
      let containerSelectorResetButton = document.querySelector(
        `.${DynamicListings.resetedFiltersQueue.shift()}`
      );
      containerSelectorResetButton.style.order = 1;
    } else if (
      DynamicListings.NumOfActiveListingFilters[`${item}-container`] === 0
    ) {
      let containerSelectorResetButton = document.querySelector(
        `.${item}-container`
      );
    ...
    if (DynamicListings.currentFilteredListings.length === 0)
      DynamicListings.messageRef.style.display = "block";
  }
  ...
  static hideUnfilteredListingContainers() {
    for (const key in DynamicListings.cache) {
      ...
      if (
        !DynamicListings.currentFilteredListings.includes(
        ...
        const index = DynamicListings.currentFilteredListings.indexOf(
          listingContainerClass
        );
        if (index >= 0)
          DynamicListings.currentFilteredListings.splice(index, 1);
      } else {
